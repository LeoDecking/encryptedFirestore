...
export class ObjectsCrypto {
    static sign(object: any, signKey: CryptoKey): PromiseLike<string> {
        ...
        return crypto.subtle.sign({ name: "ECDSA", hash: { name: "SHA-512" } }, signKey, utf8.encode(JSON.stringify(ObjectsCrypto.sortObject(object))))
            .then(signature => base64.encode(new Uint8Array(signature)));
    }

    static verify(object: any, signature: string, verifyKey: CryptoKey): PromiseLike<boolean> {
        return crypto.subtle.verify({ name: "ECFSA", hash: { name: "SHA-512" } }, verifyKey, base64.decode(signature), utf8.encode(JSON.stringify(ObjectsCrypto.sortObject(object))));
    }

    static encrypt(object: any, secretKey: CryptoKey): PromiseLike<string> {
        let iv: Uint8Array = ObjectsCrypto.getRandomBytes(12);

        return crypto.subtle.encrypt({ name: "AES-GCM", iv: iv }, secretKey, utf8.encode(JSON.stringify(ObjectsCrypto.sortObject(object))))
            .then(encrypted => base64.encode(iv) + base64.encode(new Uint8Array(encrypted)));
    }

    static decrypt(encryptedObject: string, secretKey: CryptoKey): PromiseLike<any> {
        return crypto.subtle.decrypt({ name: "AES-GCM", iv: base64.decode(encryptedObject.substr(0, 16)) }, secretKey, base64.decode(encryptedObject.substr(16)))
